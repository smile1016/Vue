<template>
  <div class="index-container">
    <div class="header">
      <Download></Download>
    </div>
    <div class="main">
      <div class="main-header">
        <div class="headIcon_wrap">
          <img
            src="data:image/png;base64,"
            alt
          />
          <p>酒店之外 就住小猪</p>
        </div>
        <div class="log_reg">
          <i class="iconfont icon-caidan"></i>
        </div>
      </div>
      <div class="search_box">
        <div id="mainBanner">
          <div class="scroll_wrap">
            <img
              src="https://image.xiaozhustatic2.com/00,600,950,1,80,2/s,3,6SPW,923,2000,2,5df460b6.jpg"
              alt
            />
          </div>
        </div>
        <div class="search_wrap">
          <div class="tabwrap">
            <div class="chosecity">
              <router-link tag='span'  class="sp" to="/city?cityname=北京">北京</router-link>
              <!-- <span class="sp">北京</span> -->
              <span>
                <i class="iconfont icon-position"></i> 我的位置
              </span>
            </div>
            <div class="chosecity">
              <router-link tag='span' class="sp" to="/calendarPage?calendarType=3">入住离开日期</router-link>
              <!-- <span class="sp">入住离开日期</span> -->
            </div>
          </div>
          <div class="btn-search">查找房源</div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
import Download from "../../components/Download";
import BScroll from "@better-scroll/core";
export default {
  components: {
    Download
  },
  created() {
    this.scroll = new BScroll("#scroll_wrap", {
      click: true,
      scrollX: true,
      probeType: 3
    });
  }
};
</script>
<style lang="scss" scoped>
@import "../../assets/style/mixin.scss";
.main {
  height: 100%;
  display: flex;
  flex-direction: column;
  //   justify-content: center;
  align-items: center;
  .main-header {
    box-sizing: border-box;
    width: 100%;
    height: 50px;
    padding: 10px 20px;
    display: flex;
    .headIcon_wrap {
      width: 100%;
      display: flex;
      img {
        width: 56px;
        height: 28px;
        margin-right: 8px;
      }
      p {
        font-size: 12px;
        color: #757575;
        line-height: 28px;
      }
    }
    .log_reg {
      i {
        font-size: 26px;
      }
    }
  }
  .search_box {
    position: relative;
    flex: 1;
    padding: 0 15px 10px;
    height: 300px;
    #mainBanner {
      .scroll_wrap {
        width: 100%;
        white-space: nowrap;
        img {
          display: inline-block;
          width: 100%;
          height: 607px;
          border-radius: 20px;
        }
      }
    }
    .search_wrap {
      width: 240px;
      height: 203px;
      padding: 25px;
      background: white;
      border-radius: 5%;
      position: absolute;
      top: 52%;
      left: 11%;
      box-shadow: 0 0.16rem 0.32rem 0 rgba(38, 40, 43, 0.6);
      .tabwrap {
        .chosecity {
          line-height: 75px;
          display: flex;
          @include border_1px(#d8d8d8);
          span {
            font-size: 12px;
            color: #212121;
          }
          .sp {
            flex: 1;
            font-size: 20px;
            color: #ff4081;
          }
        }
      }
      .btn-search {
        width: 240px;
        line-height: 45px;
        font-size: 18px;
        color: #fff;
        background: #ff4081;
        text-align: center;
        border-radius: 30px;
      }
    }
  }
}
</style>